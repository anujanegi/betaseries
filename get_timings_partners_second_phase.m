...
function out = get_timings_partners_second_phase(sbj)   
...
run_num = 6; 
 ...
 for iRun = 1:run_num  
     ...
     for iTrial = 1:nrTrials                
         ...
         time =  my_experiment.run(iRun).trial(iTrial).ins_start - my_experiment.run(iRun).start_scan.t;    
     ...
     % save onset times
     if exist(sprintf('onset_times_second_phase_sub-%02i.txt', sbj), 'file')
        save(sprintf('onset_times_second_phase_sub-%02i.txt', sbj), 'human', '-ASCII','-append')
     else
        save(sprintf('onset_times_second_phase_sub-%02i.txt', sbj), 'human', '-ASCII')
     end
     
     save(sprintf('onset_times_second_phase_sub-%02i.txt', sbj), 'robot', '-ASCII', '-append')  
     save(sprintf('onset_times_second_phase_sub-%02i.txt', sbj), 'pc', '-ASCII', '-append')
